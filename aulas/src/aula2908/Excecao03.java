	...
	public static void main(String[] args) {
		...
		while(true) {
			try {
				...
				int n = Integer.parseInt(s); //exception unchecked sendo tratada
				int a = 30 / n;
				if(n<1 || n>12) {
					JOptionPane.showMessageDialog(null, "Mês inválido!");
					//lançar exceção de mês invalido
				}else {
					JOptionPane.showMessageDialog(null, "Mês é válido");
					break;
				}

			//Exception direto, retorna qualquer tipo de exceção, mas não é recomendado, se recomenda catchs especificos.
			}catch(Exception erro) {
				//Tratamento a exceção especifica, funciona tipo catch(NumberFormatException erro)
			 ...
			 * catch(NumberFormatException erro) {
				...
				//exceção de erro de valor diferente de número para um valor int
			}catch(ArithmeticException erro) {
				JOptionPane.showMessageDialog(null,"Erro2: " + erro);
				//exceção de erro de aitmética exemplo 30/0
			}
			 *
			 */
